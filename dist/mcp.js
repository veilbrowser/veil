...
import { closeBrowser, ensureBrowser, getPage } from './browser.js';
 ...
 */
async function resolveWritableOutputPath(pathValue) {
 ...
 */
function createVeilMcpServer(browserDefaults = {}) {
    ...
    server.registerTool('navigate', {
        ...
        try {
            ...
            await page.goto(url, { waitUntil: waitUntil, timeout: timeoutMs });
    ...
    server.registerTool('current_page', {
    ...
    server.registerTool('selector_exists', {
 ...
 */
async function resolveTlsOptions(certPath, keyPath) {
    if (!certPath && !keyPath) {
        return undefined;
    }
    if (!certPath || !keyPath) {
